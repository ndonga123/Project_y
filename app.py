...
def add_patient():
    if request.method == 'POST':
        p = Patient(name=request.form.get('name'), email=request.form.get('email'), phone=request.form.get('phone'),
                    address=request.form.get('address'), age=int(request.form.get('age') or 0),
                    gender=request.form.get('gender'), diagnosis=request.form.get('diagnosis'))
...
def edit_patient(id):
    ...
    if request.method == 'POST':
        p.name = request.form.get('name'); p.email = request.form.get('email'); p.phone = request.form.get('phone')
        p.address = request.form.get('address'); p.age = int(request.form.get('age') or 0); p.gender = request.form.get('gender')
        p.diagnosis = request.form.get('diagnosis')
        db.session.commit(); flash('Saved','success'); return redirect(url_for('patients'))
    return render_template('edit_patient.html', patient=p)

@app.route('/patient/delete/<int:id>', methods=['POST'])
def delete_patient(id):
    p = Patient.query.get_or_404(id); db.session.delete(p); db.session.commit(); flash('Deleted','warning'); return redirect(url_for('patients'))
...
def add_appointment():
    pid = int(request.form.get('patient_id')); doctor = request.form.get('doctor_name'); date = request.form.get('date'); time_s = request.form.get('time'); reason = request.form.get('reason')
    a = Appointment(patient_id=pid, doctor=doctor, date=date, time=time_s, reason=reason); db.session.add(a); db.session.commit(); flash('Appointment added','success'); return redirect(url_for('appointments'))
...
def add_bill():
    pid = int(request.form.get('patient_id')); amount = float(request.form.get('amount') or 0); method = request.form.get('payment_method'); desc = request.form.get('description')
    b = Bill(patient_id=pid, amount=amount, payment_method=method, description=desc, status='Pending'); db.session.add(b); db.session.commit(); flash('Bill created','success'); return redirect(url_for('billing'))

@app.route('/bill/delete/<int:id>', methods=['POST'])
def delete_bill(id):
    b = Bill.query.get_or_404(id); db.session.delete(b); db.session.commit(); flash('Deleted','warning'); return redirect(url_for('billing'))
